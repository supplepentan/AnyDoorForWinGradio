...
def process_pairs(ref_image, ref_mask, tar_image, tar_mask, max_ratio=0.8):
    ...
    masked_ref_image = cv2.resize(masked_ref_image.astype(np.uint8), (224, 224)).astype(
        np.uint8
    )
    ...
    collage_mask = pad_to_square(collage_mask, pad_value=0, random=False).astype(
        np.uint8
    )
    ...
    collage_mask = (
        cv2.resize(collage_mask.astype(np.uint8), (512, 512)).astype(np.float32) > 0.5
    ).astype(np.float32)
...
def run_local(base, ref, *args):
    ...
    if use_interactive_seg:
        ...
        ref_mask = (
            iseg_model(img_ten, mask_ten)["instances"][0, 0].detach().numpy() > 0.5
        )

    processed_item = process_pairs(
        ref_image.copy(), ref_mask.copy(), image.copy(), mask.copy(), max_ratio=0.8
    )
    ...
    synthesis = inference_single_image(
        ref_image.copy(), ref_mask.copy(), image.copy(), mask.copy(), *args
    )
    ...
    with gr.Column():
        ...
        with gr.Row():
            ...
            with gr.Accordion("Advanced Option", open=True):
                ...
                seed = gr.Slider(
                    label="Seed", minimum=-1, maximum=999999999, step=1, value=-1
                )
                gr.Markdown(
                    " Higher guidance-scale makes higher fidelity, while lower guidance-scale leads to more harmonized blending."
                )

        gr.Markdown(
            "# Upload / Select Images for the Background (left) and Reference Object (right)"
        )
        gr.Markdown(
            "### Your could draw coarse masks on the background to indicate the desired location and shape."
        )
        gr.Markdown(
            "### <u>Do not forget</u> to annotate the target object on the reference image."
        )
        with gr.Row():
            base = gr.Image(
                label="Background",
        ...
        with gr.Row():
            with gr.Column():
                gr.Examples(
                    image_list,
                    inputs=[base],
                    label="Examples - Background Image",
            ...
            with gr.Column():
                gr.Examples(
                    ref_list,
                    inputs=[ref],
                    label="Examples - Reference Object",
                    examples_per_page=16,
                )
